...
def main():
    ...
    ax1.set_title(f'Fundus Image\nQuality: {quality_rating} ({probabilities[predicted_class]:.1%})',
                  fontsize=14, fontweight='bold', color=quality_color)
    ax1.axis('off')
    
    # Add quality badge to image
    bbox_props = dict(boxstyle='round,pad=0.5', facecolor=quality_color, alpha=0.8, edgecolor='black', linewidth=2)
    ax1.text(0.05, 0.95, quality_rating, transform=ax1.transAxes,
    ...
    bars = ax2.bar(range(3), probabilities, color=colors, alpha=0.7, edgecolor='black', linewidth=1.5)
...
🟢 Good (q3):
   ...
   for diagnosis"""
    ...
    ax3.text(0.1, 0.9, guide_text, transform=ax3.transAxes,
