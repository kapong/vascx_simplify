...
def main():
    ...
    print(
        f"   Difference:       ({abs(seq_fovea[0]-batch_fovea[0]).item():.4f}, {abs(seq_fovea[1]-batch_fovea[1]).item():.4f})"
    )
    ...
    if match_batch and match_large:
        ...
        max_diff = max(
            abs(seq_fovea[0] - batch_fovea[0]).item(), abs(seq_fovea[1] - batch_fovea[1]).item()
        )
        if max_diff < 2.0:
    ...
    print(
        f"   Large batch (50 imgs): {large_time:.2f}s ({large_time/len(large_images):.3f}s per image)"
    )
