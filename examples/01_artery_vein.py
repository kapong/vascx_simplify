...
def main():
    ...
    alpha = 0.5
    overlay[pred_classes == 1] = (1 - alpha) * overlay[pred_classes == 1] + alpha * np.array(
        [255, 0, 0]
    )
    overlay[pred_classes == 2] = (1 - alpha) * overlay[pred_classes == 2] + alpha * np.array(
        [0, 0, 255]
    )
    overlay[pred_classes == 3] = (1 - alpha) * overlay[pred_classes == 3] + alpha * np.array(
        [0, 255, 0]
    )
